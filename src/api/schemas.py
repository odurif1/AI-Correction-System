...
class DetectionRequest(BaseModel):
    """Request to start PDF detection."""
    force_refresh: bool = False
    mode: Literal["interactive", "auto"] = "interactive"


class DetectionResponse(BaseModel):
    """Response from PDF detection."""
    detection_id: str
    mode: str
    is_valid_pdf: bool
    page_count: int
...
class ConfirmDetectionRequest(BaseModel):
    """Request to confirm detection and prepare for grading."""
    confirm: bool = True
...
class ConfirmDetectionResponse(BaseModel):
    """Response after confirming detection."""
    success: bool
    session_id: str
    status: str
...
class UpdateSessionSubjectRequest(BaseModel):
    """Request to update the session subject (exam name)."""
    subject: str = Field(..., min_length=1, max_length=200, description="New session subject/exam name")
