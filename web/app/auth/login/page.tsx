...
export default function LoginPage() {
  ...
  const rawRedirect = searchParams.get("redirect") || "/sessions/new";
  // Prevent open redirect: only allow relative paths
  const redirectPath = rawRedirect.startsWith("/") && !rawRedirect.startsWith("//") ? rawRedirect : "/sessions/new";
        ...
        <Card className="w-full max-w-md border-0 shadow-2xl">
          ...
          <CardContent className="space-y-5 px-8">
            ...
            <Button
              type="button"
              variant="outline"
              className="w-full h-11 bg-white hover:bg-gray-50 opacity-60 cursor-not-allowed"
              disabled
            >
              ...
              {error && (
                <div className={`p-3 text-sm rounded-lg border flex items-center gap-2 ${
                  sessionExpired
                    ...
                    : "text-red-600 bg-red-50 dark:bg-red-950/50 border-red-100 dark:border-red-900"
                }`}>
                  {sessionExpired && <AlertCircle className="h-4 w-4 flex-shrink-0" />}
                  {error}
                </div>
              )}
